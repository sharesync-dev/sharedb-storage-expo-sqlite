...
const { validateJsonPaths } = require('@sharesync/sharedb-storage-sqlite').JsonPathValidator;
const { retryWithBackoff } = require('../utils/retry-utils');

/**
 * Detects a benign expo-sqlite finalizeAsync error caused by a SQLite
 * success code being mis-translated by the native bridge.
 *
 * Per the SQLite documentation (https://www.sqlite.org/rescode.html),
 * there are exactly three primary non-error result codes:
 *   - SQLITE_OK   = 0   ("not an error")
 *   - SQLITE_ROW  = 100 ("another row available")
 *   - SQLITE_DONE = 101 ("no more rows available")
 * Extended result codes derived from SQLITE_OK (currently just
 * SQLITE_OK_LOAD_PERMANENTLY = 256) share the low byte 0, so a mask-based
 * check `(n & 0xFF) === 0` future-proofs us against any new variants.
 *
 * None of these are real errors. The underlying step has already
 * completed and the write/read result has already been produced before
 * finalize runs, so we swallow these inside the retry boundary to avoid
 * duplicate retries of an op that already succeeded.
 *
 * Detection works on either the parsed numeric code or the official short
 * message text, since the bridge has been observed surfacing both.
 */
function isBenignFinalizeError(error) {
  if (!error || typeof error.message !== 'string') return false;
  if (!error.message.toLowerCase().includes('finalizeasync')) return false;

  // The success-code information may live on error.cause (when
  // expo-modules-core wraps the native error with ES2022 Error.cause)
  // OR be flattened into the top-level message string with a
  // "→ Caused by: …" suffix (when the bridge stringifies eagerly).
  // Scan both the cause chain and the top-level message itself.
  if (messageIndicatesBenign(error.message)) return true;

  const seen = new Set();
  let cause = error.cause;
  while (cause && typeof cause === 'object' && !seen.has(cause)) {
    seen.add(cause);
    if (typeof cause.message === 'string' && messageIndicatesBenign(cause.message)) {
      return true;
    }
    cause = cause.cause;
  }
...
function messageIndicatesBenign(message) {
  const msg = message.toLowerCase();
  const codeMatch = msg.match(/\bcode\s+(\d+)\b/);
  if (codeMatch) {
    const n = parseInt(codeMatch[1], 10);
    if ((n & 0xFF) === 0 || n === 100 || n === 101) {
      return true;
    }
  }
  return (
    msg.includes('not an error') ||              // SQLITE_OK
    msg.includes('another row available') ||     // SQLITE_ROW
    msg.includes('no more rows available')       // SQLITE_DONE
  );
}
...
function ExpoSqliteAdapter(fileName, dirPath, options = {}) {
  ...
  this.db = null;
  this.SQLite = null;
  // Serializes all async DB access on this connection so expo-sqlite's
  // prepare/execute/finalize cycles never overlap. Its New-Architecture JSI
  // statement objects race under concurrent access (the schema strategy fans out
  // reads + writes via Promise.all), surfacing as "Cannot use shared object that
  // was already released" — most readily on Android. Every async query method
  // below runs through this single promise chain.
  ...
  try {
    this.FileSystem = require('expo-file-system/legacy');
  } catch (e) {
...
ExpoSqliteAdapter.prototype.connect = function() {
  ...
  return new Promise(async function(resolve, reject) {
    try {
      // If already connected, return immediately (idempotent)
      if (adapter.db) {
        adapter.debug && console.log('[ExpoSqliteAdapter] Already connected to database: ' + adapter.fileName);
        resolve();
        return;
      }

      // For ShareDB databases, don't use a new connection if we're going to attach later
      // This allows the database to be attached to another connection
      const dbOptions = adapter.fileName.startsWith('sharedb_')
        ? {}
        : { useNewConnection: true };

      // Open the database from its explicit directory (dirPath). A bare filename is
      // resolved by expo-sqlite against its NATIVE defaultDatabaseDirectory, which
      // does NOT always match where the caller (via expo-file-system) actually placed
      // the file — notably under Expo Go on Android, where the app's per-experience
      // documentDirectory diverges from expo-sqlite's unscoped default dir, so opening
      // by name silently creates an empty database instead of the real one. Passing
      // dirPath makes expo-sqlite open the intended file. On iOS / standalone builds
      // the two directories coincide, so this is a no-op there.
      adapter.db = adapter.SQLite.openDatabaseSync(adapter.fileName, dbOptions, adapter.dirPath);
      adapter.debug && console.log('[ExpoSqliteAdapter] Connected to database: ' + adapter.fileName + ' in ' + adapter.dirPath);
...
/**
 * Serialize a DB operation on this connection's single op-chain. Each task starts
 * only after the previous one settles (fulfilled OR rejected), guaranteeing no two
 * expo-sqlite statement operations are ever in flight at once. A task's own failure
 * is surfaced to its caller but never breaks the chain for later operations.
 */
ExpoSqliteAdapter.prototype._enqueue = function(task) {
  const result = this._opChain.then(task, task);
  this._opChain = result.then(function() {}, function() {});
  return result;
};
...
ExpoSqliteAdapter.prototype.runAsync = function(sql, params) {
  ...
  return adapter._enqueue(function() { return retryWithBackoff(async function() {
    if (!adapter.db) {
    ...
    try {
      ...
      if (isBenignFinalizeError(error)) {
        adapter.debug && console.log('[ExpoSqliteAdapter] Ignored benign finalizeAsync success-code for: ' + sql.substring(0, 50));
        return { lastID: undefined, changes: undefined };
      }
      throw error;
    }
  }, {
