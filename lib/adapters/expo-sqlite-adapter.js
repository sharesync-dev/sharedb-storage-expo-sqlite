...
/**
 * Detects a benign expo-sqlite finalizeAsync error caused by SQLITE_DONE
 * (code 101, "no more rows available"). This is not a real error in SQLite —
 * it's the success sentinel that step() returns when a statement finishes.
 * The expo-sqlite native bridge sometimes re-surfaces it from finalizeAsync,
 * even though the underlying step already completed successfully and the
 * write/read result has already been produced. We swallow it inside the
 * retry boundary so we don't trigger duplicate retries of an op that
 * already succeeded.
 */
function isBenignFinalizeDoneError(error) {
  ...
  while (cause && typeof cause === 'object' && !seen.has(cause)) {
    ...
    if (msg.includes('no more rows available') || msg.includes('code 101')) {
      return true;
    }
    cause = cause.cause;
  }
...
ExpoSqliteAdapter.prototype.runAsync = function(sql, params) {
  ...
  return retryWithBackoff(async function() {
    ...
    try {
      ...
      if (isBenignFinalizeDoneError(error)) {
        adapter.debug && console.log('[ExpoSqliteAdapter] Ignored benign finalizeAsync DONE for: ' + sql.substring(0, 50));
