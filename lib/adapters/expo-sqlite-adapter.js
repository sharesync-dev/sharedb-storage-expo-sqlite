 ...
 * success code being mis-translated by the native bridge. The two we have
 * seen in the wild are:
 *   - code 101 (SQLITE_DONE, "no more rows available") — the sentinel that
 *     sqlite3_step() returns when a statement finishes
 *   - code 0   (SQLITE_OK, "not an error") — the literal "no error" code
 * Neither is a real error in SQLite. The underlying step has already
 * completed successfully and the write/read result has already been
 * produced before finalize runs, so we swallow these inside the retry
 * boundary to avoid duplicate retries of an op that already succeeded.
 */
function isBenignFinalizeError(error) {
  ...
  while (cause && typeof cause === 'object' && !seen.has(cause)) {
    ...
    if (msg.includes('no more rows available') ||
