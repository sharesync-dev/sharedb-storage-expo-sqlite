 ...
 * success code being mis-translated by the native bridge. The codes we
 * have seen in the wild are:
 *   - code 0   (SQLITE_OK,   "not an error")           — literal no-error
 *   - code 100 (SQLITE_ROW,  "another row available")  — row-is-ready
 *   - code 101 (SQLITE_DONE, "no more rows available") — statement finished
 * None of these are real errors in SQLite — they are the documented
 * primary success codes returned by sqlite3_step() and friends. The
 * underlying step has already completed successfully and the write/read
 * result has already been produced before finalize runs, so we swallow
 * these inside the retry boundary to avoid duplicate retries of an op
